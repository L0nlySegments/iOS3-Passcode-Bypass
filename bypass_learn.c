...
static void mobiledevice_restore(service_conn_t connection, CFMutableDictionaryRef files) {
    CFStringRef info = CFStringCreateWithFormat(NULL, NULL, CFSTR("[RESTORE] Starting Restore with files: %@"), files);
    printCFString(info);
...
{	
    ...
    if(CFStringFind(deviceInfo, CFSTR("3.1.3"), kCFCompareCaseInsensitive).location != kCFNotFound) {
        ...
        sleep(3); //Wait 3 seconds for restoremode to be enabled on the iDevice
